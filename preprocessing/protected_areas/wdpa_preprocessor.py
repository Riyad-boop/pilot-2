...
# local imports
from protected_areas.raster_country_code_processor import RasterCountryCodeProcessor
from protected_areas.pa_processor_wrapper import PA_Processor_Wrapper
from protected_areas.rasterizer_processor import Rasterizer_Processor
from protected_areas.update_land_impedance import UpdateLandImpedance
from protected_areas.landscape_affinity_estimator import Landscape_Affinity_Estimator
from protected_areas.lulc_pa_raster_sum import Lulc_pa_raster_sum
from protected_areas.utils import utils
...
class WDPA_Preprocessor(utils):
    ...
    def __init__(self, current_dir:str, parent_dir:str) -> None:
        ...
        self.config = self.load_yaml(os.path.join(parent_dir, 'config.yaml'))
        # # STEP 1.0: Get the unique country codes from the LULC raster data
        # unique_country_codes = self.get_lulc_country_codes()
        # # STEP 2.0: Fetch and process the protected areas for each unique country code
        # merged_gpkg = self.protected_area_to_merged_geopackage(unique_country_codes)
        # # STEP 3.0: Rasterize the merged GeoPackage file
        # self.rasterize_pas_by_year(merged_gpkg, self.config["lulc_dir"], os.path.join(self.current_dir, "pas_timeseries"))
        ...
        # self.run_shell_command(os.path.join(self.current_dir, "raster_sum_loop.sh"))

        # # STEP 5.0: Reclassify the raster
        # # TODO rewrite step 4 to python instead of shell script and then pass variables to this step
        # input_dir = r'lulc_pa'
...
if __name__ == "__main__":
    ...
    mw = WDPA_Preprocessor(current_dir, parent_dir)