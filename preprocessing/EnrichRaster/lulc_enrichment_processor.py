...
from vector_preprocessor import vector_preprocessor
from lulc_preprocessor import lulc_data_preprocessor


from utils import extract_attribute_values, merge_tiffs_into_vrt
...
class lulc_enrichment_processor():
    ...
    def prepare_lulc_osm_data(self):
        ...
        ## OSM PREPROCESSING
        self.vp = vector_preprocessor(self.config, self.parent_dir, self.vector_dir, self.year,self.lp.raster_metadata.crs_info["epsg"], self.lp.raster_metadata.is_cartesian)
        # buffer features in the input vector data
        self.vp.buffer_features('railways', self.vp.vector_railways_buffered)
        self.vp.buffer_features('roads', self.vp.vector_roads_buffered)
    ...
    def rasterize_vector_roads(self, raster_metadata:str , roads_gpkg:str, burn_value:int, groupby_roads:bool=True):
        ...
        merge_tiffs_into_vrt(road_tiffs, os.path.join(self.parent_dir,self.output_dir,f'roads_{self.year}.vrt'))
