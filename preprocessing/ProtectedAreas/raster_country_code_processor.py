...
class RasterCountryCodeProcessor():
    """
    This class preprocesses the World Database on Protected Areas (WDPA) raster data for the Land Use and Land Cover (LULC) data
    """
    ...
    def fetch_lulc_country_codes(self, save_geojson:bool=True) -> dict[set]:
        ...
        # fetch the country codes for each LULC raster and track progress
        total = len(self.lulc_s)
        for lulc in track(self.lulc_s, description="Fetching country codes..."):
            ...
            total += 1
        print(f"Countries extracted from {total} LULC rasters")
